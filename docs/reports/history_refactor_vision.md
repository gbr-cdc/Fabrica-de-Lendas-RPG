...
## 1. Problem Statement
The current implementation of the `history` list in `ActionLoad` and `AttackLoad` violates the **[ARCH.RULES.CORE.MVC] (Game-MVC)** guardrail. It stores narrative, human-readable strings (e.g., *"Warrior hit for 10 damage!"*) within the Model layer. 
